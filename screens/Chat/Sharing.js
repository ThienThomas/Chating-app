...
export default function Sharing({route}) {
  ...
  useState(() => {
    if (auth.currentUser) {
        const Query1 = query(collection(db,"chatrooms", docid, "sharingimages"), orderBy("createdAt", 'desc'))
  ...
  useState(() => {
   // let videos = []
    if (auth.currentUser) {
        const Query2 = query(collection(db,"chatrooms", docid, "sharingvideos"), orderBy("createdAt", 'desc'))
